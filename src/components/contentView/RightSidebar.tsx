...
export default function RightSidebar({ internalLinks }: { internalLinks: InternalMention[] }) {
    return (
        <aside
            className="w-64 h-full hidden xl:flex flex-col shrink-0 border-l relative z-10 p-8 bg-zinc-950 border-zinc-900 text-zinc-400 
            dark:bg-zinc-950 dark:border-zinc-900 dark:text-zinc-400 overflow-y-auto scrollbar-none"
        >
            <div className="animate-fadeIn">
                <h4 className="text-xs font-bold uppercase tracking-wider dark:text-zinc-500 text-zinc-400 mb-4 flex items-center gap-2 select-none">
                ...
                <nav className="space-y-1">
                    {internalLinks.map((mention) => (
                        <Link
                            ...
                            hover:bg-zinc-200/50 border border-transparent dark:hover:border-zinc-900"
                        >
                            <span
                                className="w-1.5 h-1.5 rounded-full dark:bg-zinc-800 bg-zinc-300 group-hover:bg-indigo-500 
                                dark:group-hover:bg-indigo-400 transition-colors shrink-0"
                            />
